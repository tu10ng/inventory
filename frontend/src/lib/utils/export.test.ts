...
describe('generateTripText', () => {
	...
	const types: Type[] = [
		...
		{ id: 2, name: '装备', sort_order: 2, parent_id: null }
	];
	...
	const tripItems: TripItemEnriched[] = [
		{
			...
			slot: { id: 1, slot_name: '冲锋衣', is_essential: true },
			candidates: []
		},
		{
	...
	const allItems: Item[] = [
		{ id: 1, type_id: 1, attrs: { name: '冲锋衣', brand: '始祖鸟' } },
		{ id: 2, type_id: 2, attrs: { name: '头灯', brand: 'Petzl' } }
	];
	...
	const groupedItems = [
		{ rootTypeName: '服装', items: [tripItems[0]] },
		{ rootTypeName: '装备', items: [tripItems[1]] }
	];

	it('includes trip name', () => {
		const text = generateTripText(trip, groupedItems, allItems, types, people, [], 1, 2);
	...
	it('includes tips', () => {
		const text = generateTripText(trip, groupedItems, allItems, types, people, tips, 1, 2);
