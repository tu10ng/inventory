...
pub async fn create(
    ...
    let row = sqlx::query_as::<_, TripItem>(
        "INSERT INTO trip_items (trip_id, item_id, custom_name, person_id, qty, checked, item_status, notes, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING *",
    )
    .bind(trip_id)
    .bind(body.item_id)
...
pub async fn update(
    ...
    let row = sqlx::query_as::<_, TripItem>(
        "UPDATE trip_items SET custom_name = ?, person_id = ?, qty = ?, checked = ?, item_status = ?, notes = ?, sort_order = ? WHERE id = ? RETURNING *",
    )
    .bind(&custom_name)
    .bind(person_id)
    .bind(qty)
